...
def fill_static_info(ws1, offers_length, start_money, use_offers):  # заполняем статическую информацию
...
def select_max_invest_potential(offers_value, current_money, offers, offers_length, use_offers, ws1, i):
    max_invest = max(map(lambda x: x[1], offers_value))
    for offer in enumerate(offers_value):
        if current_money >= offer[1][0] and offer[1][1] > 0 and offer[1][
            1] == max_invest:  # выбираем предложение с максимальной прибылью
...
def fill_day_income(ws1, use_offers, i):
...
def main():
    ...
    fill_static_info(ws1, offers_length, start_money, use_offers)
    ...
    for i in range(1, invest_duration + 1):
        for j in use_offers:
            if j["day"] >= j["no_income"] + j["start_day"]:
                ...
                j["day"] = i
        offers.sort(key=lambda x: (invest_duration - x["no_income"] - 1) * x["income"] - x["start_cost"], reverse=True)  # сортируем предложения по уменьшению эффективности инвестиций
        offers_value = get_invest_potential(offers, invest_duration - i)
        if offers_value:  # если у нас еще остались предложения
            current_money = select_max_invest_potential(offers_value, current_money, offers, offers_length, use_offers, ws1, i)
        ws1.cell(row=1, column=i + 2).value = i # записываем текущий день
        ws1.cell(row=offers_length + 3, column=i + 2).value = current_money  # записываем сумму на текущий день

        fill_day_income(ws1, use_offers, i)
