...
int main()
{
    ...
    /*
    Принцип работы алгоритма заключается в том, что на каждом шаге он «посещает» одну вершину и пытается уменьшать метки.
    Работа алгоритма завершается, когда все вершины из точки А в точку Б посещены.
    Алгоритм принимает параметры расстояний, предков, и меток. Потом выполняется n итераций, на каждом шаге итерации сперва находится
    вершина u имеющяя наименьшее расстояние, среди неотмеченных вершин, если расстояние до вершины оказывется равно -1, то алгоритм останавливается, 
    а иначе вершина отмечается как "помеченная". Далее сканируются все ребра исходящие из этой вершины и выполняются релаксации. Если расстояние
    между вершинами меняется, то оно пересчитывается и записывается в предок. По окончанию всех итераций результат расстояния всех вершин
    из точки А в точку Б записывается  в массив p[], за исключением стартовой вершины. для того чтобы отсортировать массив в нужном нам порядке
    используем цикл восстановления пути. Далее результат выводится на экран.
    */
    for (int i = 0; i < n; ++i) {
        mn = inf, u = -1;
        for (int j = 0; j < n; ++j)
    ...
    if (p[finish] == -1)
        ...
        for (int u = finish; u != -1; u = p[u]) // цикл восстановления пути
            v.push_back(u);
